
export const foods = [
  {
    id: 1,
    name: 'Idli',
    region: 'South',
    description: 'Steamed rice cakes served with chutney and sambar',
    category: 'breakfast',
    imageSrc: '/Foodi/idli.jpg',
    price: 15
  },
  {
    id: 2,
    name: 'Dosa',
    region: 'South',
    description: 'Crispy fermented crepe made from rice batter and black lentils',
    category: 'breakfast',
    imageSrc:'/Foodi/dosa.jpg',
    price: 18
  },
  {
    id: 3,
    name: 'Biryani',
    region: 'North',
    description: 'Fragrant spiced rice dish with vegetables',
    category: 'lunch',
    imageSrc:'/Foodi/biryani.jpg',
    price: 20
  },
  {
    id: 4,
    name: 'Paneer Butter Masala',
    region: 'North',
    description: 'Creamy tomato-based curry with cottage cheese',
    category: 'dinner',
    imageSrc: '/Foodi/paneer_butter_masala.jpg',
    price: 19
  },
  {
    id: 5,
    name: 'Samosa',
    region: 'North',
    description: 'Fried pastry filled with spiced potatoes and peas',
    category: 'starter',
    imageSrc:'/Foodi/samosa.jpg',
    price: 10
  },
  {
    id: 6,
    name: 'Vada Pav',
    region: 'West',
    description: 'Spicy potato fritter in a bread bun served with chutneys',
    category: 'starter',
    imageSrc: '/Foodi/vada_pav.jpg',
    price: 12
  },
  {
    id: 7,
    name: 'Pongal',
    region: 'South',
    description: 'Savory rice and lentil dish flavored with ghee and pepper',
    category: 'breakfast',
    imageSrc: '/Foodi/pongal.jpg',
    price: 14
  },
  {
    id: 8,
    name: 'Palak Paneer',
    region: 'North',
    description: 'Cottage cheese in a creamy spinach gravy',
    category: 'dinner',
    imageSrc: '/Foodi/palak_paneer.jpg',
    price: 20
  },
  {
    id: 9,
    name: 'Pani Puri',
    region: 'North',
    description: 'Crispy hollow puris filled with tangy tamarind water and potatoes',
    category: 'starter',
    imageSrc: '/Foodi/pani_puri.jpg',
    price: 11
  },
  {
    id: 10,
    name: 'Masala Dosa',
    region: 'South',
    description: 'Dosa stuffed with spiced potatoes served with chutney and sambar',
    category: 'breakfast',
    imageSrc: '/Foodi/masala_dosa.jpg',
    price: 17
  },
  {
    id: 11,
    name: 'Chole Bhature',
    region: 'North',
    description: 'Spicy chickpea curry served with deep-fried bread',
    category: 'lunch',
    imageSrc: '/Foodi/chole_bhature.jpg',
    price: 20
  },
  {
    id: 12,
    name: 'Medu Vada',
    region: 'South',
    description: 'Crispy savory donuts made from lentils served with sambar and chutney',
    category: 'breakfast',
    imageSrc: '/Foodi/medu_vada.jpg',
    price: 13
  },
  {
    id: 13,
    name: 'Aloo Gobi',
    region: 'North',
    description: 'Stir-fried potatoes and cauliflower with spices',
    category: 'lunch',
    imageSrc: ' /Foodi/aloo_gobi.jpg',
    price: 16
  },
  {
    id: 14,
    name: 'Pav Bhaji',
    region: 'West',
    description: 'Spicy mashed vegetable curry served with buttered bread rolls',
    category: 'lunch',
    imageSrc: '/Foodi/pav_bhaji.png',
    price: 18
  },
  {
    id: 15,
    name: 'Upma',
    region: 'South',
    description: 'Savory porridge made from semolina and vegetables',
    category: 'breakfast',
    imageSrc: '/Foodi/upma.png',
    price: 15
  }
  // {
  //   id: 16,
  //   name: 'Hyderabadi Biryani',
  //   region: 'South',
  //   description: 'Flavored rice with vegetables and aromatic spices',
  //   category: 'lunch',
  //   imageSrc: '../assets/hyderabadi_biryani.jpg',
  //   price: 20
  // },
  // {
  //   id: 17,
  //   name: 'Rajma',
  //   region: 'North',
  //   description: 'Red kidney bean curry served with rice',
  //   category: 'lunch',
  //   imageSrc: '../assets/rajma.jpg',
  //   price: 19
  // },
  // {
  //   id: 18,
  //   name: 'Rasam',
  //   region: 'South',
  //   description: 'Spicy soup made with tamarind, tomatoes, and spices',
  //   category: 'starter',
  //   imageSrc: '../assets/rasam.jpg',
  //   price: 12
  // },
  // {
  //   id: 19,
  //   name: 'Gulab Jamun',
  //   region: 'North',
  //   description: 'Deep-fried dough balls soaked in sugar syrup',
  //   category: 'dessert',
  //   imageSrc: '../assets/gulab_jamun.jpg',
  //   price: 14
  // },
  // {
  //   id: 20,
  //   name: 'Lassi',
  //   region: 'North',
  //   description: 'Refreshing yogurt drink, sweet or salted',
  //   category: 'drink',
  //   imageSrc: '../assets/lassi.jpg',
  //   price: 12
  // },
  // {
  //   id: 21,
  //   name: 'Aloo Paratha',
  //   region: 'North',
  //   description: 'Stuffed flatbread with spiced potato filling',
  //   category: 'breakfast',
  //   imageSrc: '../assets/aloo_paratha.jpg',
  //   price: 15
  // },
  // {
  //   id: 22,
  //   name: 'Bhindi Masala',
  //   region: 'North',
  //   description: 'Stir-fried okra with onions and tomatoes',
  //   category: 'lunch',
  //   imageSrc: '../assets/bhindi_masala.jpg',
  //   price: 17
  // },
  // {
  //   id: 23,
  //   name: 'Pesarattu',
  //   region: 'South',
  //   description: 'Green gram dosa served with ginger chutney',
  //   category: 'breakfast',
  //   imageSrc: '../assets/pesarattu.jpg',
  //   price: 18
  // },
  // {
  //   id: 24,
  //   name: 'Kofta Curry',
  //   region: 'North',
  //   description: 'Spiced vegetable balls in a creamy gravy',
  //   category: 'dinner',
  //   imageSrc: '../assets/kofta_curry.jpg',
  //   price: 20
  // },
  // {
  //   id: 25,
  //   name: 'Misal Pav',
  //   region: 'West',
  //   description: 'Spicy sprouted bean curry served with bread',
  //   category: 'lunch',
  //   imageSrc: '../assets/misal_pav.jpg',
  //   price: 14
  // },
  // {
  //   id: 26,
  //   name: 'Appam',
  //   region: 'South',
  //   description: 'Soft hoppers made from fermented rice batter and coconut milk',
  //   category: 'breakfast',
  //   imageSrc: '../assets/appam.jpg',
  //   price: 16
  // },
  // {
  //   id: 27,
  //   name: 'Dhokla',
  //   region: 'West',
  //   description: 'Steamed fermented rice and chickpea batter cakes',
  //   category: 'breakfast',
  //   imageSrc: '../assets/dhokla.jpg',
  //   price: 13
  // },
  // {
  //   id: 28,
  //   name: 'Baingan Bharta',
  //   region: 'North',
  //   description: 'Smoky mashed eggplant cooked with spices and tomatoes',
  //   category: 'dinner',
  //   imageSrc: '../assets/baingan_bharta.jpg',
  //   price: 19
  // },
  // {
  //   id: 29,
  //   name: 'Makki di Roti',
  //   region: 'North',
  //   description: 'Corn flatbread often served with sarson ka saag',
  //   category: 'lunch',
  //   imageSrc: '../assets/makki_di_roti.jpg',
  //   price: 12
  // },
  // {
  //   id: 30,
  //   name: 'Sarson ka Saag',
  //   region: 'North',
  //   description: 'Mustard greens curry served with makki di roti',
  //   category: 'lunch',
  //   imageSrc: '../assets/sarson_ka_saag.jpg',
  //   price: 16
  // },
  // {
  //   id: 31,
  //   name: 'Methi Thepla',
  //   region: 'West',
  //   description: 'Spiced fenugreek flatbread',
  //   category: 'breakfast',
  //   imageSrc: '../assets/methi_thepla.jpg',
  //   price: 14
  // },
  // {


  //   id: 32,
  //   name: 'Kadhi',
  //   region: 'North',
  //   description: 'Yogurt-based curry with gram flour dumplings',
  //   category: 'lunch',
  //   imageSrc: '../assets/kadhi.jpg',
  //   price: 15
  // },
  // {
  //   id: 33,
  //   name: 'Undhiyu',
  //   region: 'West',
  //   description: 'Mixed vegetable dish with spices and fenugreek dumplings',
  //   category: 'lunch',
  //   imageSrc: '../assets/undhiyu.jpg',
  //   price: 19
  // },
  // {
  //   id: 34,
  //   name: 'Aloo Methi',
  //   region: 'North',
  //   description: 'Potatoes stir-fried with fenugreek leaves',
  //   category: 'lunch',
  //   imageSrc: '../assets/aloo_methi.jpg',
  //   price: 16
  // },
  // {
  //   id: 35,
  //   name: 'Chana Masala',
  //   region: 'North',
  //   description: 'Spiced chickpea curry',
  //   category: 'lunch',
  //   imageSrc: '../assets/chana_masala.jpg',
  //   price: 18
  // },
  // {
  //   id: 36,
  //   name: 'Veg Pulao',
  //   region: 'North',
  //   description: 'Mixed vegetable rice dish',
  //   category: 'lunch',
  //   imageSrc: '../assets/veg_pulao.jpg',
  //   price: 20
  // },
  // {
  //   id: 37,
  //   name: 'Matar Paneer',
  //   region: 'North',
  //   description: 'Cottage cheese and peas in a spiced tomato gravy',
  //   category: 'dinner',
  //   imageSrc: '../assets/matar_paneer.jpg',
  //   price: 19
  // },
  // {
  //   id: 38,
  //   name: 'Bhature',
  //   region: 'North',
  //   description: 'Deep-fried bread usually served with chole',
  //   category: 'lunch',
  //   imageSrc: '../assets/bhature.jpg',
  //   price: 18
  // },
  // {
  //   id: 39,
  //   name: 'Sabudana Khichdi',
  //   region: 'West',
  //   description: 'Spicy tapioca pearl dish with peanuts and potatoes',
  //   category: 'breakfast',
  //   imageSrc: '../assets/sabudana_khichdi.jpg',
  //   price: 14
  // },
  // {
  //   id: 40,
  //   name: 'Aloo Bonda',
  //   region: 'South',
  //   description: 'Potato fritters coated in gram flour batter',
  //   category: 'starter',
  //   imageSrc: '../assets/aloo_bonda.jpg',
  //   price: 12
  // }
];
